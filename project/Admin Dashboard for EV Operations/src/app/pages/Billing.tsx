...
import type { BillingRecord, PaymentMethod, PaymentMethodStats } from '../types';
...
export default function Billing() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    billingApi.list({ tenantId: currentTenant?.id })
      .then(setRecords)
      .catch(() => setRecords(mockBillingRecords))
  ...
  const handleExport = (type: 'pdf' | 'csv') => {
    if (type === 'csv') {
      ...
      exportToCsv('billing', rows, currentUser?.name ?? 'Admin', currentTenant?.id ?? 'all');
      toast.success(`Exported ${rows.length} billing records to CSV`);
    } else {
      toast.success('Exporting billing data as PDF...');
    }
  };
