...
export default function Sessions() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    sessionsApi.list({ tenantId: currentTenant?.id })
      .then(setSessions)
  ...
  const handleExport = () => {
    const rows = filteredSessions.map((s) => ({
      ...
      'User Identifier': s.userIdentifier,
      'Start Time': format(s.startTime, 'yyyy-MM-dd HH:mm'),
      'Duration (min)': s.duration,
      'Energy (kWh)': s.energyKwh,
      'Cost (DZD)': s.cost,
      Status: s.status,
      Tenant: s.tenantId,
    }));
    exportToCsv('sessions', rows, currentUser?.name ?? 'Admin', currentTenant?.id ?? 'all');
    toast.success(`Exported ${rows.length} sessions to CSV`);
  };
