...
async function seedAppUsers() {
  ...
  for (const [id, email, fullName, phone, rfid] of appUsers) {
    await pool.query(
      `INSERT INTO app_users (app_user_id, email, password_hash, full_name, phone, rfid_card_number)
       VALUES (?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE
         ...
         status = 'active'`,
      [id, email, hash, fullName, phone, rfid],
    );
  }
}
    ...
    await pool.query(
      `INSERT INTO stations
       ...
       ON DUPLICATE KEY UPDATE
         ...
         current_tariff_dzd_per_kwh = VALUES(current_tariff_dzd_per_kwh)`,
      station,
    );
  }
}
