...
// Android app calls this after login to register its FCM token.
// This token is used to send push notifications to this specific device.
router.post('/fcm-token', requireAppAuth, async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    await pool.query(
      'UPDATE app_users SET fcm_token = ? WHERE app_user_id = ?',
      [fcm_token, req.appUser!.appUserId]
    );
