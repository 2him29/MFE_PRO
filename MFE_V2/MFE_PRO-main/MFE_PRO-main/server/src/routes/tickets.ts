...
router.get('/', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // Technicians see tickets assigned to them OR unassigned ones in their tenant
    // (so a new "Report a Problem" submission from the app shows up for the team).
    if (req.user!.role === 'technician') {
      q += ' AND (assigned_to_user_id = ? OR assigned_to_user_id IS NULL)';
      p.push(req.user!.userId);
    } else if (req.query.assignedToId) {
      q += ' AND assigned_to_user_id = ?';
      p.push(req.query.assignedToId as string);
    }
...
router.post('/', requireAuth, requireRole('super_admin', 'tenant_admin'), async (req: Request, res: Response) => {
  try {
    ...
    const { station_id, title, description, category, priority, assigned_to_user_id, sla_deadline, tenant_id } = parsed.data;
    ...
    await pool.query(
      `INSERT INTO tickets (ticket_id, tenant_id, station_id, created_by_user_id, assigned_to_user_id,
       ...
       VALUES (?,?,?,?,?,?,?,?,?,?)`,
      [ticket_id, targetTenantId, station_id, req.user!.userId,
