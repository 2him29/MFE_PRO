...
import { Bell, ChevronDown, LogOut, Menu, Sun, Moon } from 'lucide-react';
...
export function Header({ onMenuClick }: HeaderProps) {
  ...
  const handleNotificationClick = (notification: AppNotification) => {
    ...
    if (needsTenantSwitch) {
      ...
      setTimeout(() => setIsSwitchingTenant(false), 600);
    }
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-20 flex h-16 items-center justify-between border-b border-gray-200 bg-white px-3 shadow-sm transition-colors duration-300 dark:border-gray-800 dark:bg-gray-900 md:left-64 md:px-6">
      ...
      <div className="flex min-w-0 items-center gap-2 md:gap-4">
        ...
        <div
          className={`flex min-w-0 items-center gap-2 rounded-xl border-2 px-2 py-1.5 shadow-md transition-all hover:shadow-lg md:gap-3 md:px-4 md:py-2 ${isSwitchingTenant ? 'opacity-50 pointer-events-none' : ''}`}
          style={{
        ...
        >
          ...
          <div className="min-w-0">
            <p className="max-w-[120px] truncate text-xs font-semibold text-gray-900 dark:text-white md:max-w-none md:text-sm">
              {isSwitchingTenant ? 'Switching…' : currentTenant?.name}
            </p>
