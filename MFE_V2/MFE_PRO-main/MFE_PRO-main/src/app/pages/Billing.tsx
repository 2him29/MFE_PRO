...
import { Download, FileText, DollarSign, TrendingUp, CreditCard, Wallet, Globe, AlertTriangle, Clock, CheckCircle2, XCircle, Receipt } from 'lucide-react';
...
export default function Billing() {
  ...
  const handleExport = (fmt: 'pdf' | 'csv') => {
    if (fmt === 'pdf') {
      toast.info('PDF export requires a print-ready view — use browser Print (Ctrl+P) for now.');
      return;
    }
    ...
    exportToCsv(rows, `billing_${format(new Date(), 'yyyyMMdd_HHmm')}.csv`, 'billing', currentUser?.name ?? 'admin');
