...
interface KPICardProps {
  ...
  accentColor?: string;
  invertTrend?: boolean; // for metrics where lower is better (faults, offline, maintenance)
  onClick?: () => void;
}

export function KPICard({ title, value, trend, icon, accentColor, invertTrend, onClick }: KPICardProps) {
  ...
  return (
    <Card
      className={`relative overflow-hidden transition-shadow ${onClick ? 'cursor-pointer hover:shadow-md hover:ring-1 hover:ring-border' : ''}`}
      onClick={onClick}
    >
